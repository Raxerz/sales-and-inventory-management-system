          ...
          <?php }?>
              ...
              <div class="form-group">
                ...
                <input type="text" name="date" class="form-control pull-right active" id="reservation">
                  ...
                  <div class="form-group">
                    <div class="input-group col-md-6">
                      <button type="submit" class="btn btn-lg btn-primary pull-left" name="display">
                        Display
                      </button>
...
?>      
			...
			<table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
<?php
  if($date!=null && $cid!="all"){
	   $query=mysqli_query($con,"select * from daily_transactions natural join customer where date(date)>='$start' and date(date)<='$end' and branch_id='$branch'and cust_id='$cid'")or die(mysqli_error($con));
   } else if($date==null && $cid!="all") {
     $query=mysqli_query($con,"select * from daily_transactions natural join customer where branch_id='$branch' and cust_id='$cid'")or die(mysqli_error($con));   
   } else if($date!=null && $cid=="all"){
     $query=mysqli_query($con,"select * from daily_transactions natural join customer where date(date)>='$start' and date(date)<='$end' and branch_id='$branch'")or die(mysqli_error($con));
   } else {
     $query=mysqli_query($con,"select * from daily_transactions natural join customer where branch_id='$branch' order by date desc")or die(mysqli_error($con));       
   }
		$qty=0;$grand=0;$discount=0;
								while($row=mysqli_fetch_array($query)){
                  $grand = $grand + $row['collected'];
?>
            <tr>
            <td><?php echo $row['cust_id'];?></td>
            <td><?php echo date("M d, Y",strtotime($row['date']));?></td>    
            <td><?php echo $row['cust_first'];?></td>
            <td style="text-align:right"><?php echo number_format($row['collected'],2);
								?></td>
