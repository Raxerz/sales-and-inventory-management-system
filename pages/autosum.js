/* This script and many more are available free online at
The JavaScript Source!! http://www.javascriptsource.com
Created by: Jim Stiles | www.jdstiles.com */
function startCalc(){
  interval = setInterval("calc()",1);
}
function calc(){
  one = document.autoSumForm.total.value;
  three = (one * 1);
  document.autoSumForm.amount_due.value = three.toFixed(2);

  four = document.autoSumForm.tendered.value; 
  five=(four * 1) - (three * 1);
  document.autoSumForm.change.value = five.toFixed(2);
  document.autoSumForm.returned.value = five.toFixed(2);
}
function stopCalc(){
  clearInterval(interval);
}
function myFunction(){
	one = document.autoSumForm.total.value;
	document.autoSumForm.amount_due.value = one;
}

function addChangeReturned(){	
	document.getElementById("change_ret").value = document.getElementById("changed").value; 
}
